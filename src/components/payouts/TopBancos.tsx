...
function BancoRow({ stats }: { stats: BancoStats }) {
  return (
    <li className="flex flex-col gap-1 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
      ...
      <span className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm tabular-nums">
        ...
        <span>
          ...
          <span className="font-mono">{formatDuration(stats.p50Seconds)}</span>
        </span>
        <span>
          <span className="text-muted-foreground">P95 </span>
          <span className="font-mono">{formatDuration(stats.p95Seconds)}</span>
        </span>
      </span>
    </li>
  );
      ...
      <CardContent>
        {isEmpty ? (
          ...
          <ul className="divide-y divide-border">
            {data.top.map((b) => (
              <BancoRow key={b.medium} stats={b} />
            ))}
            {data.otros.count > 0 && (
              <BancoRow key="otros" stats={data.otros} />
            )}
          </ul>
        )}
      </CardContent>
